...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh1, dW2, db2 = affine_backward(dS, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        layer_dims = np.hstack([input_dim, hidden_dims, num_classes]).reshape(-1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          if self.normalization == "batchnorm":
            ...
            out, cache = affine_norm_relu_forward(out, w, b, gamma, beta, self.bn_params[i])
          else:
        ...
        Wlast = self.params[f'W{self.num_layers}']
        blast = self.params[f'b{self.num_layers}']
        scores, cache = affine_forward(out, Wlast, blast)
        caches.append(cache)
        ...
        # Compute loss (data loss + reg loss)
        loss, dS = softmax_loss(scores, y)
        for i in range(self.num_layers):
          w = self.params[f'W{i+1}']
          loss += 0.5 * self.reg * np.sum(w ** 2)

        # Grads for the last affine layer before the softmax 
        dh, dWlast, dblast = affine_backward(dS, caches[-1])
        grads[f'W{self.num_layers}'] = dWlast + self.reg * self.params[f'W{self.num_layers}']
        grads[f'b{self.num_layers}'] = dblast

        # Grads for all the affine_relu layers before that
        for i in range(self.num_layers-2, -1, -1):
          if self.use_dropout:
            # Remove dropout cache
            dcache = caches[i][0]
            caches[i] = caches[i][1:]
            dh = dropout_backward(dh, dcache)

          if self.normalization == "batchnorm":
            dh, dw, db, dgamma, dbeta = affine_norm_relu_backward(dh, caches[i])
...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    - gamma, beta: Parameters for the batch norm layer
