...
def gram_matrix(features, normalize=True):
    ...
    features_flattened = features.view(N, C, H * W)

    # Inefficient implementation
    #
    # for x in range(C):
    #   for y in range(C):
    #     gram[:, x, y] = torch.sum(features_flattened[:, x] * features_flattened[:, y])

    # Efficient implementation using torch's batch matrix multiplication
    # We have to transpose the channel and activation dimensions of the second matrix
    gram = torch.bmm(features_flattened, torch.transpose(features_flattened, 1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      layer_gram = gram_matrix(feats[style_layers[i]])
      layer_loss = style_weights[i] * torch.sum((layer_gram - style_targets[i])**2)
      loss += layer_loss
...
def tv_loss(img, tv_weight):
    ...
    img_roll_v = torch.roll(img, -1, dims=2)
    img_roll_h = torch.roll(img, -1, dims=3)

    ver_loss = torch.sum((img_roll_v[:, :, :-1, :] - img[:, :, :-1, :])**2)
    hor_loss = torch.sum((img_roll_h[:, :, :, :-1] - img[:, :, :, :-1])**2)

    # for i in range(1):
    #    print('hehe')

    loss = tv_weight * (ver_loss + hor_loss)
    return loss
